...
function BotCard({
  ...
  return (
    <div
      className={`bg-bg-1 border rounded-lg p-4 transition-all ${
        bot.status === "error"
          ? "border-red/30"
          : isRunning
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        ...
        <div>
          <div className="text-2xs text-text-3 mb-0.5">Open Trades</div>
          <div className="text-sm font-bold text-text-0">{openCount}</div>
        </div>
      </div>
...
export default function DashboardPage() {
  ...
  // Portfolio aggregates (D-1 to D-5)
  ...
  const [todayClosedCount, setTodayClosedCount] = useState<number>(0);
  ...
  // D-D: selected timeframe for daily chart
  ...
  // New data display sections state
  ...
  const loadData = useCallback(async (showLoadingState = false) => {
    ...
    try {
      ...
      if (selectedBotId === null && botList.length > 0) {
        setSelectedBotId(botList[0].id);
      }

      // D-24: Fetch AI agreement rate (non-blocking)
      fetchAIAgreementRate(7)
        .then((agr) => {
          const a = agr as { all_agree_pct?: number; strong_disagree?: number; enabled?: boolean };
          setAiAgreementPct(a.all_agree_pct ?? null);
          setAiStrongDisagree(a.strong_disagree ?? 0);
          setAiEnabled(true);
        })
        .catch(() => {
          setAiEnabled(false);
      ...
      // Portfolio balance for equity
      try {
        const pb = await portfolioBalance();
        setTotalEquity(pb.total_value);
      } catch {
        // Fall back to sum
      }

      // Portfolio profit for unrealized
      try {
        const pp = await portfolioProfit();
        setTotalUnrealized(pp.combined.profit_all_coin - pp.combined.profit_closed_coin);
      } catch {
        // nonblocking
      }
      ...
      try {
        ...
        if (totalUnrealized === null) setTotalUnrealized(unrealized);

        const todayClosed = pt.trades.filter(
          (t) => !t.is_open && isToday(t.close_date)
        );
        setClosedTrades(todayClosed);
        setTodayClosedCount(todayClosed.length);
        const todayP = todayClosed.reduce((s, t) => s + (t.close_profit_abs ?? 0), 0);
        setTodayRealized(todayP);
      } catch {
        // open trades failed — non-blocking
      }

      // Per-bot: profit, sparkline
        ...
        runningBots.map(async (bot) => {
          try {
            ...
            if (d.status === "fulfilled") {
              sparks[bot.id] = d.value.data.map((item) => item.abs_profit);
            }
            if (s.status === "fulfilled") {
              openCounts[bot.id] = s.value.filter((t) => t.is_open).length;
            }
          } catch {
            // per-bot error is isolated
          }
        })
      );
      ...
      await Promise.allSettled(
        runningBots.map(async (bot) => {
          try {
            const stats = await botStats(bot.id);
            const dd = (stats.max_drawdown ?? 0) * 100;
      ...
      if (runningBots.length > 0) {
        try {
          const daily = await botDaily(runningBots[0].id, dailyPeriod);
      ...
      // Compute win rate from all profits
      ...
      try {
        ...
        setRiskEvents(events.slice(0, 5));
      } catch {
        // nonblocking
      }
  ...
  useEffect(() => {
    loadData(true);
    const interval = setInterval(() => loadData(false), REFRESH_INTERVALS.DASHBOARD);
    return () => clearInterval(interval);
  }, [loadData]);

  // ── Load detail sections when selectedBotId changes ────────────────────

  const loadBotDetails = useCallback(async (botId: number) => {
    const m = mountedRef; // Capture ref for mount-safe callbacks
    // Weekly
  ...
  async function handleBotAction(action: string, fn: () => Promise<unknown>) {
    if (!selectedBotId) {
  ...
  // Running bots only for section D-8
  ...
  return (
    <AppShell title="Dashboard">
      ...
      {staleWarning && (
        <div className="mb-4 px-4 py-3 bg-amber-bg border border-amber/20 rounded-btn flex items-center justify-between">
          ...
          <button
            ...
            className="text-xs text-amber underline cursor-pointer hover:no-underline"
          >
            Retry now
          </button>
        </div>
      ...
      <Card className="mb-6">
        <CardHeader
          ...
          action={
            <div className="flex items-center gap-2">
              <label className="text-2xs text-text-3 uppercase tracking-wide">Bot:</label>
              <select
                value={selectedBotId ?? ""}
                onChange={(e) => setSelectedBotId(e.target.value ? Number(e.target.value) : null)}
                className="bg-bg-1 border border-border rounded px-2 py-1 text-xs text-text-0 focus:outline-none focus:border-accent"
              >
        ...
        <CardBody className="p-4">
          <div className="flex flex-wrap gap-2">
            ...
            <button
              type="button"
              disabled={!selectedBotId || controlLoading !== null}
              onClick={() => handleBotAction("Start Bot", () => startBot(selectedBotId!))}
            ...
            >
              {controlLoading === "Start Bot" ? "..." : "Start"}
            </button>
            {/* 2. Stop Bot */}
            <button
              type="button"
              disabled={!selectedBotId || controlLoading !== null}
              onClick={() => handleBotAction("Stop Bot", () => stopBot(selectedBotId!))}
            ...
            >
              {controlLoading === "Stop Bot" ? "..." : "Stop"}
            </button>
            {/* 3. Stop New Entries */}
            <button
              type="button"
              disabled={!selectedBotId || controlLoading !== null}
              onClick={() => handleBotAction("Stop Buy", () => botStopBuy(selectedBotId!))}
            ...
            >
              {controlLoading === "Stop Buy" ? "..." : "Stop New Entries"}
            </button>
            {/* 4. Pause */}
            <button
              type="button"
              disabled={!selectedBotId || controlLoading !== null}
              onClick={() => handleBotAction("Pause", () => botPause(selectedBotId!))}
              className="px-3 py-1.5 text-xs font-semibold rounded border border-amber/30 text-amber bg-amber-bg hover:bg-amber/[0.18] disabled:opacity-50 cursor-pointer transition-all"
            >
              {controlLoading === "Pause" ? "..." : "Pause"}
            </button>
            {/* 5. Force Entry */}
            <button
              type="button"
            ...
            <button
              type="button"
              disabled={!selectedBotId || controlLoading !== null}
              onClick={() => handleBotAction("Reload Config", () => reloadBotConfig(selectedBotId!))}
            ...
            >
              {controlLoading === "Reload Config" ? "..." : "Reload Config"}
            </button>
          </div>
          {selectedBot && (
            <div className="mt-3 text-2xs text-text-3">
              Selected: <span className="text-text-1 font-medium">{selectedBot.name}</span>
              {" "}&middot;{" "}
      ...
      <div className="grid grid-cols-5 gap-4 mb-6">
        {loading ? (
          ...
          <>
            ...
            />
            {/* D-3: Today's Realized */}
            <StatCard
              label="Today's Realized"
              icon="✅"
              value={todayRealized != null ? fmtMoney(todayRealized) : "\u2014"}
              valueColor={profitColor(todayRealized)}
              sub={`${todayClosedCount} closed today`}
            />
            {/* D-4: Max Drawdown (30d) */}
            <StatCard
              label="Max Drawdown (30d)"
              icon="📉"
              value={maxDrawdown != null ? `-${fmt(maxDrawdown, 1)}%` : "\u2014"}
              valueColor={maxDrawdown != null && maxDrawdown > 10 ? "text-red" : maxDrawdown != null && maxDrawdown > 5 ? "text-amber" : "text-green"}
              sub="Peak-to-trough, all bots"
            />
            {/* D-5: Win Rate */}
            <StatCard
              label="Win Rate"
      ...
      {/* ════ D-24: AI Agreement Badge ════ */}
      {aiEnabled && aiAgreementPct !== null && (
        <div
          className="mb-4 flex items-center gap-3 px-4 py-3 rounded-btn"
          style={{
            background:
              aiStrongDisagree > 0
                ? "rgba(239,68,68,.08)"
        ...
        >
          <span style={{ fontSize: "1.25rem" }}>
            {aiStrongDisagree > 0 ? "⚠️" : "🤖"}
          </span>
          <div className="flex-1">
            <span
              className="text-xs font-semibold"
              style={{ color: aiStrongDisagree > 0 ? "#ef4444" : "#818cf8" }}
            >
              AI Agreement (7d):{" "}
              <strong>{aiAgreementPct.toFixed(1)}%</strong> all-agree
              {aiStrongDisagree > 0 && (
                <span style={{ color: "#ef4444" }}>
                  {" "}&middot; {aiStrongDisagree} strong disagreement
                  {aiStrongDisagree > 1 ? "s" : ""}
                </span>
              )}
            </span>
            {aiStrongDisagree > 0 && (
              <p className="text-2xs text-text-3 mt-0.5">
                Claude and Grok both disagreed with FreqAI on{" "}
                {aiStrongDisagree} recent signal{aiStrongDisagree > 1 ? "s" : ""}. Review in AI Insights.
              </p>
            )}
          </div>
          <a
      ...
      {/* ════ D-6, D-7: Bot Health Grid ════ */}
      {!loading && bots.length > 0 && (
        <div className="mb-6">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-text-0">Bot Overview</h2>
            <span className="text-2xs text-text-3">
              {runningBotCards.length} of {bots.length} running
            </span>
          </div>
          <div className="grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-3">
            {loading
              ? Array.from({ length: 4 }).map((_, i) => <SkeletonBotCard key={`skel-bot-${i}`} />)
              : bots.map((bot) => (
                  <BotCard
                    key={bot.id}
                    bot={bot}
      ...
      {/* ════ D-11, D-12, D-13: Open Positions Table ════ */}
      <Card className="mb-6">
        <CardHeader
          ...
          action={
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="text-xs text-accent hover:text-accent cursor-pointer font-medium"
              >
                ↻ Refresh
              </button>
            </div>
        ...
        {loading ? (
          ...
          <CardBody>
            <div className="py-8 text-center text-sm text-text-3">No running bots</div>
          </CardBody>
        ) : openTrades.length === 0 ? (
          <CardBody>
            <div className="py-8 text-center text-sm text-text-3">No open positions</div>
          </CardBody>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  {["Pair", "Bot", "Side", "Open Rate", "Current", "P&L", "Duration", "Actions"].map((h) => (
                    <th
                      key={h}
                      className="text-left px-4 py-3 text-2xs font-semibold text-text-3 uppercase tracking-wider border-b border-border whitespace-nowrap"
                    >
                      {h}
                    </th>
                  ))}
                  ...
                  return (
                    <React.Fragment key={trade.trade_id}>
                      <tr
                        ...
                        onClick={() =>
                          setExpandedTradeId(
                            isExpanded ? null : trade.trade_id
                          )
                        }
                      >
                        {/* D-11: Pair */}
                        <td className="px-4 py-3 text-xs font-semibold text-text-0 whitespace-nowrap">
                          {trade.pair}
                        </td>
                        {/* Bot */}
                        ...
                        {/* open_rate */}
                        <td className="px-4 py-3 text-xs text-text-2 font-mono whitespace-nowrap">
                          {fmt(trade.open_rate, 4)}
                        </td>
                        {/* current_rate */}
                        <td className="px-4 py-3 text-xs text-text-1 font-mono whitespace-nowrap">
                          {fmt(trade.current_rate, 4)}
                        </td>
                        {/* P&L */}
                        <td className={`px-4 py-3 text-xs font-bold whitespace-nowrap ${profitColor(pnl)}`}>
                          {pnl != null ? `${pnl >= 0 ? "+" : ""}${fmt(pnl, 2)}` : "\u2014"}
                          {pct != null && (
                            <span className="text-2xs font-normal ml-1 opacity-70">
                              ({(pct * 100).toFixed(2)}%)
                            </span>
                          )}
                        </td>
                        ...
                        <td className="px-4 py-3 text-xs text-text-3 whitespace-nowrap">
                          {fmtDuration(trade.open_date)}
                        </td>
                        {/* Actions: Force Exit + Delete Trade + Reload Trade + Cancel Open Order */}
                        <td className="px-4 py-3 whitespace-nowrap">
                          <div className="flex gap-1" onClick={(e) => e.stopPropagation()}>
                            <button
                              type="button"
                              onClick={() => handleForceExit(trade)}
                              disabled={isExiting}
                            ...
                            <button
                              type="button"
                              onClick={() => handleReloadTrade(trade)}
                              ...
                              title="Reload Trade from Exchange"
                            >
                              Reload
                            </button>
                            <button
                              ...
                              onClick={() => {
                                toast.warning(
                                  `Delete trade #${trade.trade_id} (${trade.pair})?`,
                                  { action: { label: "DELETE", onClick: () => handleDeleteTrade(trade) } }
                                );
                              }}
                            ...
                            {hasOpenOrder && (
                              <button
                                type="button"
                                onClick={() => handleCancelOpenOrder(trade)}
                      ...
                      {/* D-12: Expanded detail row */}
                      {isExpanded && (
                        <tr key={`exp-${trade.trade_id}`}>
                          <td colSpan={8} className="bg-bg-1 border-b border-border px-4 py-3">
      ...
      {/* ════ D-15 + D-17: Daily P&L + Equity Curve ════ */}
          ...
          {loading ? (
            ...
            <CardBody>
              <div className="flex items-end gap-1 h-28">
                {dailyData.slice(-14).map((item, i) => {
                  const val = item.abs_profit;
                  const max = Math.max(
                    ...dailyData.map((x) => Math.abs(x.abs_profit)),
                    0.01
                  );
                  ...
                  return (
                    <div key={item.date ?? `day-${i}`} className="flex-1 flex flex-col items-center gap-0.5" title={`${item.date.slice(5)}: ${val >= 0 ? "+" : ""}${val.toFixed(2)} USDT`}>
                      <div
                        className={`w-full rounded-[2px] ${val >= 0 ? "bg-green" : "bg-red"} opacity-70`}
                        style={{ height: `${pct}%` }}
                      />
                    </div>
                  );
              ...
              <div className="mt-2 text-right text-2xs text-text-3">
                Total (all bots): {" "}
                <span className={`font-bold ${profitColor(totalClosedProfit)}`}>
                  {fmtMoney(totalClosedProfit)}
                </span>
              </div>
            </CardBody>
          )}
              ...
              <ResponsiveContainer width="100%" height={120}>
                <AreaChart data={chartData} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
                  ...
                  <Tooltip
                    ...
                    formatter={(v: unknown) => {
                      const n = Number(v);
                      return [`${n >= 0 ? "+" : ""}${n.toFixed(2)} USDT`, "Cum. P&L"];
                    }}
                  />
                  <Area
                    type="monotone"
                ...
                {weeklyData && weeklyData.data.length > 0 ? (
                  <ResponsiveContainer width="100%" height={140}>
                    <BarChart data={weeklyData.data.map((w) => ({ ...w, date: w.date.slice(5) }))}>
                      ...
                      <Tooltip
                        contentStyle={{ background: "#12121c", border: "1px solid #1e1e30", borderRadius: 6, fontSize: 11 }}
            ...
            <SectionLoader loading={monthlyLoading} error={monthlyError}>
              <CardBody className="p-3">
                {monthlyData && monthlyData.data.length > 0 ? (
                  <ResponsiveContainer width="100%" height={140}>
                    <BarChart data={monthlyData.data.map((m) => ({ ...m, date: m.date.slice(0, 7) }))}>
                      <XAxis dataKey="date" tick={{ fontSize: 9, fill: "#55556a" }} axisLine={false} tickLine={false} />
                      <YAxis tick={{ fontSize: 9, fill: "#55556a" }} axisLine={false} tickLine={false} width={40} />
                      <Tooltip
                        contentStyle={{ background: "#12121c", border: "1px solid #1e1e30", borderRadius: 6, fontSize: 11 }}
      ...
      {selectedBotId && (
        <div className="grid grid-cols-2 gap-5 mb-6">
          {/* 3. Per-pair Performance */}
          <Card>
            <CardHeader title="Per-pair Performance" icon="📊" />
            <SectionLoader loading={perfLoading} error={perfError}>
              {perfData.length === 0 ? (
                <CardBody>
                  <div className="py-4 text-center text-sm text-text-3">No performance data</div>
                </CardBody>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse">
              ...
              {statsData ? (
                <CardBody>
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div>
                      <span className="text-text-3 block text-2xs uppercase tracking-wide mb-0.5">profit_factor</span>
                      <span className="font-mono text-text-1 font-bold">{fmt(statsData.profit_factor, 2)}</span>
                    </div>
                    <div>
                      <span className="text-text-3 block text-2xs uppercase tracking-wide mb-0.5">trading_volume</span>
                      <span className="font-mono text-text-1">{fmt(statsData.trading_volume, 2)}</span>
                    </div>
                    <div>
                      <span className="text-text-3 block text-2xs uppercase tracking-wide mb-0.5">Wins / Losses / Draws</span>
                      <span className="font-mono">
                        <span className="text-green">{statsData.wins}</span>
                        {" / "}
                        <span className="text-red">{statsData.losses}</span>
                        {" / "}
                        <span className="text-text-3">{statsData.draws}</span>
                      </span>
                    </div>
                    <div>
                      <span className="text-text-3 block text-2xs uppercase tracking-wide mb-0.5">max_drawdown</span>
                      <span className="font-mono text-red">{statsData.max_drawdown != null ? `${(statsData.max_drawdown * 100).toFixed(2)}%` : "\u2014"}</span>
                    </div>
                    <div>
                      <span className="text-text-3 block text-2xs uppercase tracking-wide mb-0.5">Avg Duration (wins)</span>
                      <span className="font-mono text-text-1">{statsData.durations.wins != null ? `${(statsData.durations.wins / 60).toFixed(1)}m` : "\u2014"}</span>
                    </div>
                    <div>
                      <span className="text-text-3 block text-2xs uppercase tracking-wide mb-0.5">Avg Duration (losses)</span>
                      <span className="font-mono text-text-1">{statsData.durations.losses != null ? `${(statsData.durations.losses / 60).toFixed(1)}m` : "\u2014"}</span>
                    </div>
                    <div>
                      <span className="text-text-3 block text-2xs uppercase tracking-wide mb-0.5">Max Consecutive Wins</span>
                      <span className="font-mono text-green">{statsData.max_consecutive_wins ?? "\u2014"}</span>
                    </div>
                    <div>
                      <span className="text-text-3 block text-2xs uppercase tracking-wide mb-0.5">Max Consecutive Losses</span>
                      <span className="font-mono text-red">{statsData.max_consecutive_losses ?? "\u2014"}</span>
                    </div>
                    <div>
                      <span className="text-text-3 block text-2xs uppercase tracking-wide mb-0.5">rejected_signals</span>
                      <span className="font-mono text-text-1">{statsData.rejected_signals}</span>
                    </div>
                    <div>
                      <span className="text-text-3 block text-2xs uppercase tracking-wide mb-0.5">timedout_entry_orders</span>
                      <span className="font-mono text-text-1">{statsData.timedout_entry_orders}</span>
                    </div>
                  </div>
                </CardBody>
              ) : (
                <CardBody>
                  <div className="py-4 text-center text-sm text-text-3">No stats data</div>
                </CardBody>
              )}
            </SectionLoader>
          </Card>
      ...
      {/* ════ Entry Tag Analysis + Exit Reason Analysis ════ */}
      {selectedBotId && (
        <div className="grid grid-cols-2 gap-5 mb-6">
          {/* 4. Entry Tag Analysis */}
          <Card>
            <CardHeader title="Entry Tag Analysis" icon="🏷" />
            <SectionLoader loading={entryLoading} error={entryError}>
              {entryData.length === 0 ? (
                <CardBody>
                  <div className="py-4 text-center text-sm text-text-3">No entry tag data</div>
                </CardBody>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse">
                    <thead>
                      <tr>
                        {["enter_tag", "Entries", "Wins", "Losses", "winrate", "profit_abs"].map((h) => (
                          <th key={h} className="text-left px-4 py-2 text-2xs font-semibold text-text-3 uppercase tracking-wider border-b border-border">{h}</th>
                        ))}
                      </tr>
                    </thead>
          ...
          <Card>
            ...
            <SectionLoader loading={exitLoading} error={exitError}>
              {exitData.length === 0 ? (
                <CardBody>
                  <div className="py-4 text-center text-sm text-text-3">No exit reason data</div>
                </CardBody>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse">
                    <thead>
                      <tr>
                        {["exit_reason", "Exits", "Wins", "Losses", "winrate", "profit_abs"].map((h) => (
                          <th key={h} className="text-left px-4 py-2 text-2xs font-semibold text-text-3 uppercase tracking-wider border-b border-border">{h}</th>
                        ))}
                      </tr>
                    </thead>
      ...
      {selectedBotId && (
        <Card className="mb-6">
          ...
          <SectionLoader loading={mixTagLoading} error={mixTagError}>
            {mixTagData.length === 0 ? (
              <CardBody>
                <div className="py-4 text-center text-sm text-text-3">No mix tag data</div>
              </CardBody>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full border-collapse">
                  <thead>
                    <tr>
                      {["enter_tag", "exit_reason", "Trades", "Wins", "Losses", "winrate", "profit_abs"].map((h) => (
                        <th key={h} className="text-left px-4 py-2 text-2xs font-semibold text-text-3 uppercase tracking-wider border-b border-border">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {mixTagData.slice(0, 30).map((m, i) => (
      ...
      {/* ════ D-18, D-19: Recent Alerts + System Health ════ */}
      <div className="grid grid-cols-2 gap-5 mb-6">
        ...
        <Card>
          <CardHeader
            ...
            icon="🔔"
            action={
              <a href="/risk" className="text-xs text-accent hover:text-accent font-medium">
                View All →
              </a>
            }
          />
          <CardBody className="p-0">
            {riskEvents.length === 0 ? (
              ...
              riskEvents.map((ev) => (
                <div
                  key={ev.id}
                  className="flex items-start gap-3 px-4 py-3 border-b border-border/40 last:border-b-0 hover:bg-bg-3 transition-colors"
                >
                  <div
                    className={`w-2 h-2 rounded-full shrink-0 mt-1.5 ${
                      ev.kill_type === "HARD_KILL" ? "bg-red" : "bg-amber"
                    }`}
                  />
                  <div className="flex-1 min-w-0">
                    <div className="text-xs font-medium text-text-0 truncate">
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 gap-2">
                {bots.map((bot) => (
                  <div
                    key={bot.id}
                    className="flex items-center gap-3 px-3 py-2 bg-bg-1 border border-border rounded-lg"
                  >
                    <div
                      className={`w-2 h-2 rounded-full shrink-0 ${
                        bot.status === "running" && bot.is_healthy
                          ? "bg-green shadow-[0_0_6px_#22c55e]"
                          : bot.status === "error" || !bot.is_healthy
      ...
      {/* ════ Bot Config Display + System Info ════ */}
      {selectedBotId && (
        <div className="grid grid-cols-2 gap-5 mb-6">
          {/* 8. Bot Config Display */}
          <Card>
            ...
            <SectionLoader loading={configLoading} error={configError}>
              {configData ? (
                <CardBody>
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div>
                      <span className="text-text-3 block text-2xs uppercase tracking-wide mb-0.5">strategy</span>
                      <span className="font-mono text-text-1 font-bold">{configData.strategy}</span>
                    </div>
                    <div>
                      <span className="text-text-3 block text-2xs uppercase tracking-wide mb-0.5">exchange</span>
                      <span className="font-mono text-text-1">{configData.exchange}</span>
                    </div>
                    <div>
                      <span className="text-text-3 block text-2xs uppercase tracking-wide mb-0.5">timeframe</span>
                      <span className="font-mono text-text-1">{configData.timeframe}</span>
                    </div>
                    <div>
                      <span className="text-text-3 block text-2xs uppercase tracking-wide mb-0.5">trading_mode</span>
                      <span className="font-mono text-text-1">{configData.trading_mode}</span>
                    </div>
                    <div>
                      <span className="text-text-3 block text-2xs uppercase tracking-wide mb-0.5">stake_currency</span>
                      <span className="font-mono text-text-1">{configData.stake_currency}</span>
                    </div>
                    <div>
                      <span className="text-text-3 block text-2xs uppercase tracking-wide mb-0.5">stake_amount</span>
                      <span className="font-mono text-text-1">{configData.stake_amount}</span>
                    </div>
                    <div>
                      <span className="text-text-3 block text-2xs uppercase tracking-wide mb-0.5">max_open_trades</span>
                      <span className="font-mono text-text-1">{configData.max_open_trades}</span>
                    </div>
                    <div>
                      <span className="text-text-3 block text-2xs uppercase tracking-wide mb-0.5">stoploss</span>
                      <span className="font-mono text-red">{(configData.stoploss * 100).toFixed(1)}%</span>
                    </div>
                    <div>
                      <span className="text-text-3 block text-2xs uppercase tracking-wide mb-0.5">dry_run</span>
                      <span className={`font-mono ${configData.dry_run ? "text-amber" : "text-green"}`}>{configData.dry_run ? "Yes" : "No"}</span>
                    </div>
                    <div>
                      <span className="text-text-3 block text-2xs uppercase tracking-wide mb-0.5">trailing_stop</span>
                      <span className="font-mono text-text-1">{configData.trailing_stop ? "On" : "Off"}</span>
                    </div>
                  </div>
                </CardBody>
              ) : (
                <CardBody>
                  <div className="py-4 text-center text-sm text-text-3">No config data</div>
                </CardBody>
              )}
            </SectionLoader>
          </Card>
          ...
          <Card>
            ...
            <SectionLoader loading={sysinfoLoading} error={sysinfoError}>
              {sysinfoData ? (
                  ...
                  <div className="grid grid-cols-2 gap-4 text-xs">
                        ...
                        {sysinfoData.cpu_pct.map((pct, i) => (
                          <div key={`cpu-${i}`} className="flex-1">
                            <div className="h-2 bg-bg-3 rounded-full overflow-hidden">
                              <div
                                className={`h-full rounded-full ${pct > 80 ? "bg-red" : pct > 50 ? "bg-amber" : "bg-green"}`}
                                style={{ width: `${pct}%` }}
                              />
                            </div>
                          </div>
                    ...
                    <div>
                      ...
                      <div className="h-2 bg-bg-3 rounded-full overflow-hidden mb-1">
                        <div
                          className={`h-full rounded-full ${sysinfoData.ram_pct > 80 ? "bg-red" : sysinfoData.ram_pct > 50 ? "bg-amber" : "bg-green"}`}
                          style={{ width: `${sysinfoData.ram_pct}%` }}
                        />
                      </div>
                ...
                <CardBody>
                  <div className="py-4 text-center text-sm text-text-3">No system info</div>
                </CardBody>
              )}
            </SectionLoader>
          </Card>
            ...
            <div className="max-h-64 overflow-y-auto bg-bg-1 font-mono text-2xs p-4">
              {logsData && logsData.logs.length > 0 ? (
                logsData.logs.map((log, i) => {
                  // FT logs format: [timestamp, level, name, message, ?extra]
                  const level = log[1] ?? "";
              ...
              <CardBody>
                {whitelistData && whitelistData.whitelist.length > 0 ? (
                  <div className="flex flex-wrap gap-1.5">
                    {whitelistData.whitelist.map((pair) => (
                      <span
                        key={pair}
                        className="text-2xs font-mono bg-bg-1 border border-border rounded px-2 py-1 text-text-1"
                      >
                        {pair}
                      </span>
                    ))}
                ...
                {whitelistData && (
                  <div className="mt-2 text-2xs text-text-3">
                    {whitelistData.length} pair{whitelistData.length !== 1 ? "s" : ""} | Method: {whitelistData.method.join(", ")}
                  </div>
                )}
              </CardBody>
            </SectionLoader>
          ...
          <Card>
            ...
            <SectionLoader loading={locksLoading} error={locksError}>
              {locksData && locksData.locks.length > 0 ? (
                ...
                <CardBody>
                  <div className="py-4 text-center text-sm text-text-3">No active locks</div>
                </CardBody>
              )}
            </SectionLoader>
          </Card>
      ...
      <Card>
        <CardHeader
          ...
          action={
            <span className="text-xs text-text-3">
              {closedTrades.length} trade{closedTrades.length !== 1 ? "s" : ""} closed today
            </span>
          }
        />
        {loading ? (
          ...
          <CardBody>
            <div className="py-8 text-center text-sm text-text-3">No trades closed today</div>
          </CardBody>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
                  ...
                  <tr key={trade.trade_id} className="hover:bg-bg-3 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded uppercase ${
                        trade.is_short ? "bg-red-bg text-red" : "bg-green-bg text-green"
                      }`}>
                        {trade.is_short ? "Short" : "Long"}
                      </span>
                    </td>
                    ...
                    <td className={`px-4 py-3 text-xs font-bold ${profitColor(trade.close_profit_abs)}`}>
                      {trade.close_profit_abs != null
                        ? `${trade.close_profit_abs >= 0 ? "+" : ""}${fmt(trade.close_profit_abs, 2)} USDT`
                        : "\u2014"}
                    </td>
