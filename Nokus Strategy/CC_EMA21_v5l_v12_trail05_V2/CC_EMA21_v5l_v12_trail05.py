...
v11 — 1% risk sizing + 3% SL hard cap (based on v10):
  Isto kao v10 ali SL cap smanjen na 3% od entry.
  Tighter cap = manje losera koji prolaze, ali i više prerano zatvorenih trades.
...
from freqtrade.strategy import IStrategy, stoploss_from_absolute, IntParameter, DecimalParameter
...
class CC_EMA21_v5l_v12_trail05(IStrategy):
    ...
    buy_rsi_high = IntParameter(45, 65, default=55, space="buy",  optimize=True)
    sell_rsi_low  = IntParameter(35, 55, default=45, space="sell", optimize=True)
    sell_rsi_high = IntParameter(55, 75, default=65, space="sell", optimize=True)

    # EMA pullback tolerance (how far low/high can deviate from EMA21)
    buy_pull_tol  = DecimalParameter(1.000, 1.012, default=1.004, decimals=3, space="buy",  optimize=True)
    sell_pull_tol = DecimalParameter(0.988, 1.000, default=0.996, decimals=3, space="sell", optimize=True)

    # ATR multiplier for swing SL distance
    atr_sl_mult = DecimalParameter(0.1, 0.8, default=0.3, decimals=2, space="buy", optimize=True)
    ...
    def custom_stake_amount(self, current_time, current_rate: float,
        ...
        """1% risk sizing: gubitak po tradu = 1% kapitala. SL cap 2%."""
        pair = kwargs.get('pair', self.config['exchange']['pair_whitelist'][0])
        dataframe, _ = self.dp.get_analyzed_dataframe(pair, self.timeframe)
        if dataframe is None or len(dataframe) == 0:
            return proposed_stake

        entry_row = dataframe.iloc[-1]
        sl_col = 'short_sl' if side == 'short' else 'long_sl'
        sl_price = float(entry_row.get(sl_col, 0))

        if sl_price <= 0 or current_rate <= 0:
            return proposed_stake

        if side == 'short':
            risk_per_unit = sl_price - current_rate
        else:
            risk_per_unit = current_rate - sl_price

        # Hard cap: ne dozvoli SL dalje od 2% od entry
        risk_per_unit = min(risk_per_unit, current_rate * 0.02)

        if risk_per_unit <= 0:
            return proposed_stake

        total_equity = self.wallets.get_total_stake_amount()
        quantity = (total_equity * 0.01) / risk_per_unit
        stake = quantity * current_rate
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["pull_bull"] = (
            (dataframe["low"] <= dataframe["ema21"] * self.buy_pull_tol.value) &
            (dataframe["close"] > dataframe["ema21"])
        ).astype(int)
        dataframe["pull_bear"] = (
            (dataframe["high"] >= dataframe["ema21"] * self.sell_pull_tol.value) &
            (dataframe["close"] < dataframe["ema21"])
        ).astype(int)
        ...
        dataframe["rsi_sell"] = (
            (dataframe["rsi"] >= self.sell_rsi_low.value) &
        ...
        dataframe["long_sl"] = dataframe["swing_low"] - dataframe["atr14"] * self.atr_sl_mult.value
        dataframe["short_sl"] = dataframe["swing_high"] + dataframe["atr14"] * self.atr_sl_mult.value
    ...
    def custom_stoploss(self, pair: str, trade: Trade, current_time: datetime,
        ...
        # 5% hard cap na SL
        if trade.is_short:
            sl_price = min(sl_price, trade.open_rate * 1.05)
        else:
            sl_price = max(sl_price, trade.open_rate * 0.95)
        ...
        if trade.is_short:
            ...
            if current_rate <= tp_1r:
                if activated_at is None:
                    ...
                    initial_trail = tp_1r + entry_atr * 0.5
                    ...
                    trail_sl = trade.min_rate + entry_atr * 0.5
            ...
            if current_rate >= tp_1r:
                if activated_at is None:
                    ...
                    initial_trail = self._trail_initial_stop.get(cache_key, tp_1r - entry_atr)
                    trail_sl = trade.max_rate - entry_atr * 0.5
